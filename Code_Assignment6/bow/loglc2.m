...
%  MAPPINGS, DATASETS, LDC, FISHERC
    ...
    if ~exist('minFunc', 'file')
        prwarning(1, 'LOGLC2 requires the minFunc optimizer. Please download it from www.di.ens.fr/~mschmidt/Software/minFunc.html and add it to the Matlab path.');
    end
    ...
    elseif isa(A, 'prdataset') && isa(W, 'prmapping') 
...
% Gradient of L2-regularized logistic regressor.
    ...
    if nargout > 1
        ...
        % Compute gradient
        dC = -[pos_E(:) - neg_E(:); pos_E_bias(:) - neg_E_bias(:)] ./ N + 2 .* lambda .* x;
    end    
end
